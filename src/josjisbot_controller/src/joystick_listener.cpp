#include "josjisbot_controller/joystick_listener.hpp"
#include <functional>
#include <memory>
...
JoystickListener::JoystickListener(const std::string& name)
: Node(name)
{
	...
	this->timer_ = this->create_wall_timer(
		5ms,
		std::bind(&JoystickListener::timerCallback, this)
	);

}

void JoystickListener::autoModeCallback() {
}

void JoystickListener::externalModeCallback() {

}

void JoystickListener::testingModeCallback() {
	this->joy_msgs_.header.set__stamp(this->get_clock()->now());
...
void JoystickListener::timerCallback() {
...
int main (int argc, char *argv[]) {
	...
	auto node = std::make_shared<JoystickListener>("josjisbot_joy_listener");
	rclcpp::spin(node);
	rclcpp::shutdown();
	return 0;
}
